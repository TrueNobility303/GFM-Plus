...
# running with
# nohup python -u train.py --data_file a9a/w8a/covtype/ijcnn1 --epochs=10
# nohup python -u train.py --data_file mushrooms --epochs=50
# nohup python -u train.py --data_file phishing --epochs=200

parser = argparse.ArgumentParser(description='Gradient Free Method')
parser.add_argument('--path', default='/home/truenobility303/truenobility303/Data/LIBSVM', type=str, help='path of data')
...
parser.add_argument('--seed', default= 42, type=int, help = 'random seed number')
parser.add_argument('--print_freq', default=10, type=int, help='frequency to print train stats')
parser.add_argument('--out_fig', default= './nonconvex/', type=str, help = 'path of output picture')
...
def main():
    ...
    if not os.path.exists(args.out_fig):
        os.makedirs(args.out_fig)
        
    with open(args.out_fig + args.data_file + '.' + str(args.seed) + '.SGD.query.txt', 'w') as f:
        print(querySGD, file=f)
    with open(args.out_fig + args.data_file + '.' + str(args.seed) + '.SGD.cost.txt', 'w') as f:
        print(costSGD, file=f)
    with open(args.out_fig + args.data_file + '.' + str(args.seed) + '.SPIDER.query.txt', 'w') as f:
        print(querySPIDER, file=f)
    with open(args.out_fig + args.data_file + '.' + str(args.seed) + '.SPIDER.cost.txt', 'w') as f:
        print(costSPIDER, file=f)
    ...
    plt.tight_layout()
    plt.savefig(args.out_fig + args.data_file + '.' + str(args.seed) +  '.png')
    plt.savefig(args.out_fig + args.data_file + '.' + str(args.seed) + '.eps', format = 'eps')
    plt.show()
